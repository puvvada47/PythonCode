...
HIVE_LOAD_ACTION_NAME = "Step_7___Hive_Text_To_ORC"
...
def step_1_generate_actions(module_name, table_list, scn, properties):
    ...
    if scn:
        scn_query = " AS OF SCN ${wf:actionData('Step_4___Get_SCN_NUMBER')['scn']}"
        ...
        if PLACEHOLDER_FIRST_HIVE_QUERY_PLACE not in line:
            ...
            if "#" not in line:
                ...
                if key_value.__len__() > 1:
                    ...
                    new = properties.get(old, "")
                    if new.__len__() > 0:
                        hive_file_write.write(line.replace(old, new))
        else:
            hive_file_write.write('echo "use ${hive_database_4_data};" >> create_hive_tables.txt' + "\n")
            for table in table_list:
                hive_file_write.write(
                    "hive --silent=true --showHeader=false --outputformat=dsv --hivevar v_tbl_name=" + table + " --hivevar v_db_name=${hive_database_4_metadata} --hivevar v_metatable_name=${hive_table_4_metadata} -f workflow/scripts/generate_create_table_statment.hql >> create_hive_tables.txt" + "\n")
